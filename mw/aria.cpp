...
int encrypt(Byte *mk, const char *src, const char *dst)
{
	...
	int nReadLength = ARIA_INPUT_UNIT_SZ;
    ...
    if ((fdDst = open(dst, O_CREAT | O_TRUNC | O_RDWR, S_IRUSR | S_IWUSR | S_IXUSR)) == -1) {
    	close(fdSrc);
    	return -1;
    }
	checkPayloadLength = htonl((size_t)size);
	if (write(fdDst, &checkPayloadLength, sizeof(checkPayloadLength)) != sizeof(checkPayloadLength)) {
		close(fdSrc);
		close(fdDst);
		return -1;
	}
	...
	while (size > 0) {
		if (size < ARIA_INPUT_UNIT_SZ) {
			memset(buf, 0, ARIA_INPUT_UNIT_SZ);
			nReadLength = size;
		}

		if (read(fdSrc, buf, nReadLength) != nReadLength) {
			close(fdSrc);
			close(fdDst);
			return -1;
		}
		...
		Crypt(buf, 12, rk, out);
		if (write(fdDst, out, ARIA_INPUT_UNIT_SZ) != ARIA_INPUT_UNIT_SZ) {
			close(fdSrc);
			close(fdDst);
			return -1;
		}
...
int decrypt(Byte *mk, const char *src, const char *dst)
{
	...
	while (size > 0) {
		...
		if (write(fdDst, out, nWriteLength) != nWriteLength) {
			close(fdSrc);
			close(fdDst);
			return -1;
		}
